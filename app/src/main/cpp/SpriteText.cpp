...
void SpriteText::init() {
    ...
    Asset *asset = new Asset("fonts/arial.ttf");
...
*/
        ...
        Characters.insert(std::pair<GLchar, Character2>(c, character));
    }
...
void SpriteText::Render(glm::mat4 MVP, std::string text, GLfloat x, GLfloat y, GLfloat scale, glm::vec3 color) {
    ;
    ...
    _LOGE("FONT 2000.1");
    _LOGE("FONT 2000.1 %s", text.c_str());
...
void SpriteText::setPos(GLfloat x, GLfloat y) {
    posX = x;
    posY = y;

}

void SpriteText::setTexture(glm::vec4 pColor) {

}

void SpriteText::setPosX(GLfloat x) {
    posX = x;
}
void SpriteText::setPosY(GLfloat y) {
    posY += y*acc;
}
void SpriteText::setPosZ(GLfloat z) {
    posZ = z;
}

GLfloat SpriteText::getPosX() {
    return posX;
}
GLfloat SpriteText::getPosY() {
    return posY;
}